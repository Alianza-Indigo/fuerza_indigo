...
/** Fotografía institucional: solo la carga quien también puede emitir. */
export function CredentialPhotoForm({
  credentialId,
  hasPhoto,
}: {
  credentialId: string;
  ...
  const [estado, accion, pendiente] = useActionState(setCredentialPhotoAction, INICIAL);
  const inputId = `foto-${credentialId}`;

  return (
    <form action={accion} encType="multipart/form-data" className="space-y-2">
      ...
      <label htmlFor={inputId} className="block text-sm font-medium">
        {hasPhoto ? 'Reemplazar fotografía' : 'Subir fotografía'}
      </label>
      <input
        id={inputId}
        name="photo"
        ...
        className="block w-full max-w-xs text-sm file:mr-3 file:min-h-11 file:rounded-lg file:border file:border-[var(--color-line-strong)] file:bg-[var(--color-surface)] file:px-3 file:font-medium"
      />
      <p id={`${inputId}-ayuda`} className="text-xs text-[var(--color-ink-soft)]">
        JPG, PNG o WebP, máximo 5 MB. Usa una fotografía frontal y reciente.
      </p>
      {estado.fieldErrors?.['photo']?.map((error) => (
        <p key={error} className="text-sm text-[var(--color-danger)]">{error}</p>
      ))}
      <SubmitButton variant="secondary">
        {pendiente ? 'Guardando…' : hasPhoto ? 'Reemplazar foto' : 'Guardar foto'}
      </SubmitButton>
