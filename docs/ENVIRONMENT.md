...
| `ACCOUNT_ACTIVATION_DELIVERY` | Entrega del enlace para establecer contraseña. `panel` lo muestra al personal en invitaciones y directamente al solicitante al terminar un registro público; `email` restablece el envío. | `panel` · `email` | Opc. (`panel`) | Opc. (`panel`) | Opc. (`panel`) |
