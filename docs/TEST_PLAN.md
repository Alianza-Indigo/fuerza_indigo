...
**Fase 7 — cubierto.** Recorre `F-15`. **Asertos:** la ficha muestra nombre, logotipo cuando lo hay, descripción y público; una ficha sin dirección configurada no muestra botón —ni uno deshabilitado—; quien pulsa el acceso lo sabe **por el texto del enlace**, no por un icono; el enlace abre el dominio externo con `rel="noopener noreferrer"` y sin un solo parámetro en la dirección; y cambiar la dirección desde el gestor se ve en el sitio público sin desplegar nada. **Sobre la caída de la plataforma externa:** no se simula, porque medir un tiempo de espera no probaría nada —un dominio inexistente falla al instante—. Se comprueba algo más fuerte y que sí puede fallar: que servir el catálogo **no hace ninguna llamada de red**, contándolas. Sin llamada no hay caída ajena que pueda bloquear el portal.
